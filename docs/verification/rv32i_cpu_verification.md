...
The verification focused on both normal instruction execution and integration between the individual processor modules.
...
The testbench checks internal Register File and Data Memory values to verify the results produced by the processor.
...
The troubleshooting process demonstrated that several apparent CPU failures were caused by the verification environment rather than the processor RTL.
